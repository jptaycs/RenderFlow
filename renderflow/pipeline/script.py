...
- The underlying fact must be real, specific, and checkable — you're only
  ...
  script could actually be written about truthfully.
...
def generate_topic_only(
    ...
    write the full script if the user likes it — replacing the old
    single-call generate_topic_idea, which always wrote a full script
    immediately even though most ideas were probably going to be discarded
    ...
    try:
        ...
        log.error("LLM returned JSON that does not match the topic-only schema")
        raise
    return idea, result


TOPIC_SCRIPT_SYSTEM_PROMPT = """\
You write short-documentary narration scripts for a YouTube trivia channel
("did you know" style true stories).

Rules:
- Write ONLY the narration script for the exact video title given below —
  do not invent a different title or change the underlying subject.
- The script must read naturally spoken aloud: no headings, no markdown,
  no scene numbers, no "Scene 1:" prefixes.
...
def build_topic_script_prompt(title: str, length_minutes: float = 1.5) -> str:
    target_words = _topic_idea_target_words(length_minutes)
    return (
        f'Write the narration script for this video title: "{title}"\n\n'
...
def generate_topic_script(
    llm: LLMProvider, title: str, length_minutes: float = 1.5
) -> tuple[GeneratedTopicScript, LLMResult]:
    """Step 2 of the "🎲 Random topic" flow: the full narration script for a
    title the user has already seen and asked for (see generate_topic_only
    for step 1). Same word-count scaling as the old combined call."""
    target_words = _topic_idea_target_words(length_minutes)
    result = llm.complete(
        TOPIC_SCRIPT_SYSTEM_PROMPT,
        build_topic_script_prompt(title, length_minutes),
        json_schema=GeneratedTopicScript.model_json_schema(),
        max_tokens=_topic_idea_max_tokens(target_words),
    )
    try:
        script = GeneratedTopicScript.model_validate_json(result.text)
    except ValidationError:
        log.error("LLM returned JSON that does not match the topic-script schema")
        raise
